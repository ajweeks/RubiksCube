...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	// Half Lambert
	diffuseIntensity = pow(diffuseIntensity * 0.5 + 0.5, 2);
	
	diffuseColor *= diffuseIntensity;
	if (gUseTextureDiffuse)
	{
		float3 sampledColor = gTextureDiffuse.Sample(gTextureSampler, texCoord);
...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	{
		float sampledGlossiness = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord);
		specularColor *= sampledGlossiness;
	}
	
	float3 halfVector = -normalize(viewDirection + gLightDirection);
	float specularStrength = dot(halfVector, normal);
	specularStrength = saturate(specularStrength); // Must saturate before raising to a power!
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specularStrength = dot(reflectedVector, viewDirection);
	specularStrength = saturate(specularStrength); // Must saturate before raising to a power!
	specularStrength = pow(specularStrength, gShininess);
	specularColor *=  specularStrength;
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	sampledNormal = mul(sampledNormal, localAxis);
...
{
	if (!gUseFresnelFalloff) return float3(0, 0, 0);
	
	float3 reflectedViewDir = reflect(viewDirection, normal);
	...
	{
		float3 sampledEnvColor = gCubeEnvironment.Sample(gTextureSampler, reflectedViewDir);
		return fresnel * fresnelMask * sampledEnvColor;
	}
	else
	{
...
{
	if (!gUseTextureEnvironment) return float3(0, 0, 0);
	
	float3 refractedVector = refract(viewDirection, normal, gRefractionIndex);
	float3 reflectedVector = reflect(refractedVector, normal);
		
	float3 sampledEnvironmentColor = gCubeEnvironment.Sample(gTextureSampler, reflectedVector);
	
	float3 finalColor = sampledEnvironmentColor * gReflectionStrength;
...
float CalculateOpacity(float2 texCoord)
{
	...
	float alpha = gTextureOpacity.Sample(gTextureSampler, texCoord) * gOpacityIntensity;
